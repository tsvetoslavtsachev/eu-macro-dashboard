...
from core.primitives import apply_transform, robust_stats_latest, _infer_yoy_periods, momentum_signal, severity_tier
...
def _dev_sigma(val: float, med: float, scale: float, polarity: Any) -> float:
    ...
    σ носи РАЗМЕРА. |dev_sigma| захранва severity_tier. Виж item F. При momentum
    скоринг val/med/scale идват от момент-базиса → σ-магнитудът на шока."""
    if scale == 0 or np.isnan(scale):
        return 0.0
    if isinstance(polarity, tuple) and polarity and polarity[0] == "U":
        center = float(polarity[2]) if polarity[1] == "target" else med
...
def score_series(
    ...
    *,
    ...
    # „неутрално 50" при реален всеисторически екстремум. Fallback: норма от ПЪЛНАТА
    # история на score_basis; ако и тя е константа → degenerate (неутрално + флаг).
    # Огледало на china scorer (percentile-ът минава на пълната история → честен етикет).
    win = _trailing_window(transformed, used_window)
    scale_fallback = False
    basis_win = _trailing_window(score_basis, used_window)
    if (scale == 0 or np.isnan(scale)) and len(score_basis) > len(basis_win):
        med_f = float(score_basis.median())
        mad_f = float((score_basis - med_f).abs().median())
    ...
    # z_raw = суров (над/под медианата) робастен z — полярностно-агностичен. z_score
    # остава oriented (=health_z). O3 правило 5: едно име = едно значение.
