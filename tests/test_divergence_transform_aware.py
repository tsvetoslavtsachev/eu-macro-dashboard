...
EU анатомия: HICP сериите са ВЕЧЕ в темп-форма (transform=level) — при тях
дефектът не се проявяваше. Засегнат беше ЗАПЛАТНИЯТ slot на stagflation_test
(EA_COMP_PER_EMPLOYEE / EA_WAGES_SALARIES — yoy_pct индекси): номинални
компенсационни индекси растат почти винаги → „натиск от заплати" беше
структурно up. Гейтът тук е върху slot_a.
"""
...
class TestMandateGate:
    def test_rising_wage_index_cooling_yoy_is_not_up(self):
        """ГЕЙТ (REVIEW-03 т.0.1, EU): растящ компенсационен индекс със спадащ
        YoY → slot_a (натиск от заплати) НЕ е up.

        Legacy raw shadow за същите данни е "up" (1.0) — дефектът, който
        държеше заплатния натиск структурно включен.
        """
        stag = _stag_pair()
        yoy_keys = _yoy_keys_in_slot(stag["slot_a"])
        assert yoy_keys, "slot_a на stagflation_test няма yoy_pct серии — сценарият е невалиден"

        snap: dict = {}
        for e in series_by_lens(stag["slot_a"]["lens"]):
            if e.get("peer_group") in stag["slot_a"]["peer_groups"]:
                snap[e["_key"]] = rising_index_cooling_rate()

        breadth_a, raw_a, _, _ = _aggregate_slot_breadth(stag["slot_a"], snap)

        assert breadth_a < 0.4, (
            f"Transform-aware breadth трябва да чете охлаждане (<0.4), получихме {breadth_a}"
        )
        assert raw_a == pytest.approx(1.0), (
            f"Суровият shadow трябва да покаже стария дефект (1.0), получихме {raw_a}"
        )

    def test_level_series_unchanged_by_transform(self):
        """level slot (fragmentation sovereign spreads): transform-aware == raw."""
        frag = next(p for p in CROSS_LENS_PAIRS if p["id"] == "fragmentation_risk")
        slot = frag["slot_b"]  # 5 sovereign спреда, всички level
        keys = [
